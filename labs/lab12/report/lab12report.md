...
Я создал файл project.sh. В этом файле я написал скрипт ,который при запуске будет делать резервную копию самого себя (то есть файла, в котором содержится его исходный код) в другую директорию backup в домашнем каталоге. Затем я создал исполняемый файл и запускал его: 
...
Я проверил с помощью cat,что копия содержит скрипт из оригинала:

![Проверка копии файла](image/5.PNG){#fig:005 width=70%}

Создал другой файл project1.sh. Написал код, обрабатывающий любое произвольное число аргументов командной строки. Сделал его исполняемый и запускал его:
...
Создал файл project2.sh и в нем написал программу - аналог команды ls. Она выводит информацию о возможностях доступа к файлам этого каталога и  информацию о нужном каталоге:
...
Создал файл project3.sh и в нем написал код, который олучает в качестве аргумента командной строки формат файла (.txt, .doc, .jpg, .pdf и т.д.) и вычисляет количество таких файлов в указанной директории.
